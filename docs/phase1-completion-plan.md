...
6. **Production runs on the VM (clawbot-v2); dev/review flows through git.** Agents read **frozen fixtures** — gitignored, since they hold licensed vendor data (repo is PUBLIC) — not a live store; a fixture's committed **content-hash lock** (`data/fixtures/locks/`) lets the review side verify the same inputs without the raw data leaving. **Gate proofs** (G0.3/G0.5, readouts) ARE committed — by the VM's autonomous run to `results/` or by the dev box — never left VM-only.
